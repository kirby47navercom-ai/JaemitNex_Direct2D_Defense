...
void SpaceDefanseGameImpl::SetMessage(const std::wstring& message)
{
    m_message = message;
...
void SpaceDefanseGameImpl::OpenEscapeMenu()
{
    if (m_escapeMenuOpen)
    {
        return;
...
void SpaceDefanseGameImpl::CloseEscapeMenu()
{
    if (!m_escapeMenuOpen)
    {
        return;
...
void SpaceDefanseGameImpl::AdjustEscapeMenuSpeed(float delta)
{
...
UiSliderDragTarget SpaceDefanseGameImpl::SliderDragTargetAt(Vec2 pos) const
{
...
void SpaceDefanseGameImpl::BeginSliderDrag(Vec2 pos)
{
...
void SpaceDefanseGameImpl::UpdateSliderDrag(Vec2 pos, bool playFeedback)
{
    if (m_activeSliderDrag == UiSliderDragTarget::None)
    {
        return;
...
void SpaceDefanseGameImpl::EndSliderDrag()
{
    if (m_activeSliderDrag != UiSliderDragTarget::None)
    {
        SaveProgress();
...
void SpaceDefanseGameImpl::ApplySliderDragValue(UiSliderDragTarget target, Vec2 pos, bool playFeedback)
{
    switch (target)
    {
...
bool SpaceDefanseGameImpl::IsInteractivePoint(Vec2 pos) const
{
    if (m_escapeMenuOpen)
    {
...
void SpaceDefanseGameImpl::OnLeftClick(Vec2 pos)
{
    // Every screen owns its click handling. The shared rectangle helpers live in
    // SpaceDefanseGameImplLayout.cpp, so render and input stay pixel-aligned.
...
void SpaceDefanseGameImpl::OnMenuClick(Vec2 pos)
{
    for (int i = 0; i < kStageCount; ++i)
    {
        if (Contains(MenuStageRect(i), pos))
...
void SpaceDefanseGameImpl::OnBriefingClick(Vec2 pos)
{
...
void SpaceDefanseGameImpl::OnShopClick(Vec2 pos)
{
    if (Contains(ShopBackButtonRect(), pos))
    {
        ResetToMenu();
...
void SpaceDefanseGameImpl::OnOptionsClick(Vec2 pos)
{
...
void SpaceDefanseGameImpl::OnEscapeMenuClick(Vec2 pos)
{
    const auto sliderValue = [](D2D1_RECT_F rect, Vec2 point) {
        const float width = std::max(1.0f, rect.right - rect.left);
        return Clamp01((point.x - rect.left) / width);
...
void SpaceDefanseGameImpl::OnResultClick(Vec2 pos)
{
...
void SpaceDefanseGameImpl::TryBuyOrUpgradeUnit(PlayerUnit unit)
{
...
void SpaceDefanseGameImpl::SetLoadoutUnit(PlayerUnit unit)
{
...
void SpaceDefanseGameImpl::OnKeyDown(WPARAM key)
{
    if (m_escapeMenuOpen)
    {
