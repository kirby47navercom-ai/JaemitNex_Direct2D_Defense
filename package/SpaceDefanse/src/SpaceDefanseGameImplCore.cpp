...
SpaceDefanseGameImpl::~SpaceDefanseGameImpl()
{
...
HRESULT SpaceDefanseGameImpl::Initialize()
{
    ...
    const wchar_t className[] = L"SpaceDefanseWindow";
    WNDCLASSEXW wc = {};
    wc.cbSize = sizeof(wc);
    wc.lpfnWndProc = SpaceDefanseGameImpl::WndProc;
...
LRESULT CALLBACK SpaceDefanseGameImpl::WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    if (message == WM_NCCREATE)
    {
        auto* create = reinterpret_cast<CREATESTRUCTW*>(lParam);
        auto* app = reinterpret_cast<SpaceDefanseGameImpl*>(create->lpCreateParams);
        SetWindowLongPtrW(hwnd, GWLP_USERDATA, reinterpret_cast<LONG_PTR>(app));
        app->m_hwnd = hwnd;
    }

    auto* app = reinterpret_cast<SpaceDefanseGameImpl*>(GetWindowLongPtrW(hwnd, GWLP_USERDATA));
    if (app)
    {
...
LRESULT SpaceDefanseGameImpl::HandleMessage(UINT message, WPARAM wParam, LPARAM lParam)
{
    switch (message)
    {
    case WM_DESTROY:
...
HRESULT SpaceDefanseGameImpl::CreateTextFormats()
{
...
HRESULT SpaceDefanseGameImpl::CreateDeviceResources()
{
    if (m_renderTarget)
    {
        return S_OK;
...
void SpaceDefanseGameImpl::DiscardDeviceResources()
{
    DiscardBitmapAssets();
    m_brush.Reset();
...
void SpaceDefanseGameImpl::RegisterPrivateFonts()
{
...
void SpaceDefanseGameImpl::UnregisterPrivateFonts()
{
...
std::wstring SpaceDefanseGameImpl::ExecutableDir() const
{
...
std::wstring SpaceDefanseGameImpl::AssetPath(const std::wstring& relativePath) const
{
...
bool SpaceDefanseGameImpl::StartMusicNow(const std::wstring& absolutePath, bool loop, float fadeLevel)
{
...
void SpaceDefanseGameImpl::PlayMusicTrack(const std::wstring& relativePath, float fadeSeconds, bool loop)
{
...
void SpaceDefanseGameImpl::StartBackgroundMusic()
{
...
std::wstring SpaceDefanseGameImpl::StageMusicPath(int stageIndex) const
{
...
void SpaceDefanseGameImpl::StartStageMusic()
{
    PlayMusicTrack(StageMusicPath(m_selectedStage), 0.70f, true);
    StartDangerMusicLayer();
}

std::wstring SpaceDefanseGameImpl::ResultMusicPath(bool victory) const
{
    return victory ? L"assets\\music\\result_victory.wav" : L"assets\\music\\result_defeat.wav";
}

void SpaceDefanseGameImpl::StartResultMusic(bool victory)
{
    StopDangerMusicLayer();
    PlayMusicTrack(ResultMusicPath(victory), 0.62f, true);
}

void SpaceDefanseGameImpl::PlayMusicStinger(const std::wstring& relativePath, float volumeScale)
{
...
void SpaceDefanseGameImpl::StartDangerMusicLayer()
{
...
void SpaceDefanseGameImpl::StopDangerMusicLayer()
{
...
float SpaceDefanseGameImpl::MusicDangerTarget() const
{
...
void SpaceDefanseGameImpl::UpdateMusicSystem(float dt)
{
    if (m_musicFadingOut)
    {
        m_musicFadeTimer += dt;
...
void SpaceDefanseGameImpl::SyncMusicVolume()
{
...
float SpaceDefanseGameImpl::VolumeForSfxKind(SfxKind kind) const
{
    // 메뉴 클릭음은 UI 볼륨, 전투에서 나는 나머지 소리는 효과음 볼륨을 따른다.
    return kind == SfxKind::Ui ? m_uiVolume : m_sfxVolume;
}

void SpaceDefanseGameImpl::PlaySfx(SfxKind kind, float minGapSeconds)
{
...
void SpaceDefanseGameImpl::PlaySfxAt(SfxKind kind, float worldX, float minGapSeconds, float volumeScale)
{
    std::wstring fileName;
    switch (kind)
    {
...
void SpaceDefanseGameImpl::PlaySfxFile(const std::wstring& relativeFileName, SfxKind throttleKind, float minGapSeconds)
{
...
void SpaceDefanseGameImpl::PlaySfxFileAt(const std::wstring& relativeFileName, SfxKind throttleKind, float worldX, float minGapSeconds, float volumeScale)
{
    const float volume = VolumeForSfxKind(throttleKind);
    if (volume <= 0.001f)
    {
...
std::wstring SpaceDefanseGameImpl::AttackSfxPath(const Unit& attacker) const
{
...
float SpaceDefanseGameImpl::AttackSfxVolumeScale(const Unit& attacker) const
{
...
void SpaceDefanseGameImpl::PlayAttackSfxAt(const Unit& attacker, float minGapSeconds)
{
...
void SpaceDefanseGameImpl::AdjustSfxVolume(float delta)
{
...
void SpaceDefanseGameImpl::AdjustUiVolume(float delta)
{
...
void SpaceDefanseGameImpl::AdjustBgmVolume(float delta)
{
...
void SpaceDefanseGameImpl::ResetAudioVolumes()
{
...
HRESULT SpaceDefanseGameImpl::LoadBitmapFromFile(const std::wstring& path, ID2D1Bitmap** bitmap) const
{
...
void SpaceDefanseGameImpl::LoadBitmapAssets()
{
    ...
    LoadBitmapFromFile(AssetPath(L"assets\\ui\\space_defanse_ui_atlas.png"), m_uiAtlas.ReleaseAndGetAddressOf());

    m_bitmapAssetsLoaded = true;
}

void SpaceDefanseGameImpl::DiscardBitmapAssets()
{
    m_vfxAtlas.Reset();
...
const StageDefinition SpaceDefanseGameImpl::CurrentStage() const
{
    return GetStageDefinition(m_selectedStage);
}

int SpaceDefanseGameImpl::UnitIndex(PlayerUnit unit) const
{
    return std::max(0, std::min(kRosterCount - 1, static_cast<int>(unit)));
}

bool SpaceDefanseGameImpl::IsUnitUnlocked(PlayerUnit unit) const
{
    return m_unitUnlocked[UnitIndex(unit)];
}

int SpaceDefanseGameImpl::UnitLevel(PlayerUnit unit) const
{
    return m_unitLevels[UnitIndex(unit)];
}

bool SpaceDefanseGameImpl::IsUnitEvolved(PlayerUnit unit) const
{
    return IsUnitUnlocked(unit) && UnitLevel(unit) >= kMaxUnitLevel;
}

std::wstring SpaceDefanseGameImpl::UnitDisplayName(PlayerUnit unit) const
{
    const UnitStats base = GetPlayerStats(unit);
    return IsUnitEvolved(unit) ? L"진화 " + base.name : base.name;
}

UnitStats SpaceDefanseGameImpl::PlayerStats(PlayerUnit unit) const
{
...
int SpaceDefanseGameImpl::UnitUnlockCost(PlayerUnit unit) const
{
...
int SpaceDefanseGameImpl::UnitUpgradeCost(PlayerUnit unit) const
{
    const int level = UnitLevel(unit);
...
int SpaceDefanseGameImpl::StageClearReward(bool firstClear) const
{
...
float SpaceDefanseGameImpl::DifficultyThreatMultiplier() const
{
    switch (m_difficulty)
    {
...
float SpaceDefanseGameImpl::DifficultyRewardMultiplier() const
{
    switch (m_difficulty)
    {
    case Difficulty::Easy:
...
std::wstring SpaceDefanseGameImpl::DifficultyLabel() const
{
    switch (m_difficulty)
    {
    case Difficulty::Easy:
...
bool SpaceDefanseGameImpl::IsStageUnlocked(int index) const
{
...
int SpaceDefanseGameImpl::HighestUnlockedStage() const
{
    int highest = 0;
...
void SpaceDefanseGameImpl::SelectStage(int index)
{
...
void SpaceDefanseGameImpl::GrantStageReward()
{
...
bool SpaceDefanseGameImpl::HasAnyProgressFile() const
{
    const auto hasGameplayProgress = [](const std::wstring& path) {
        ...
        file >> header;
        int lumen = 0;
        if (header == L"SPACE_DEFANSE_PROGRESS_V2")
        {
            file >> lumen;
        }
        else
...
void SpaceDefanseGameImpl::BeginStoryCrawl(bool autoContinueToMenu, GameScreen returnScreen, bool returnToEscapeMenu)
{
...
void SpaceDefanseGameImpl::FinishStoryCrawl()
{
    m_storyTimer = 0.0f;
    m_sceneTransitionTimer = m_sceneTransitionMax;
...
void SpaceDefanseGameImpl::BeginEndingScene()
{
...
void SpaceDefanseGameImpl::FinishEndingScene()
{
    m_endingTimer = 0.0f;
...
std::wstring SpaceDefanseGameImpl::ProgressPath() const
{
    return ProgressPath(m_saveSlot);
}

std::wstring SpaceDefanseGameImpl::ProgressPath(int slot) const
{
    ...
    const std::wstring fileName = L"space_defanse_progress_slot" + ToWideInt(safeSlot + 1) + L".txt";
    if (slash == std::wstring::npos)
    {
        return fileName;
    }
...
std::wstring SpaceDefanseGameImpl::LegacyProgressPath() const
{
    wchar_t path[MAX_PATH] = {};
    GetModuleFileNameW(nullptr, path, MAX_PATH);
    std::wstring fullPath = path;
    ...
    if (slash == std::wstring::npos)
    {
        return L"space_defanse_progress.txt";
    }
    return fullPath.substr(0, slash + 1) + L"space_defanse_progress.txt";
}

std::wstring SpaceDefanseGameImpl::SaveSlotLabel() const
{
    return L"슬롯 " + ToWideInt(m_saveSlot + 1);
}

void SpaceDefanseGameImpl::LoadProgress()
{
    LoadProgress(m_saveSlot);
}

void SpaceDefanseGameImpl::LoadProgress(int slot)
{
    ...
    std::wstring header;
    file >> header;
    const bool version2 = header == L"SPACE_DEFANSE_PROGRESS_V2";
    if (version2)
    {
        file >> m_lumen;
    }
...
void SpaceDefanseGameImpl::SaveProgress()
{
...
void SpaceDefanseGameImpl::SaveProgressToSlot(int slot) const
{
    ...
    file << L"SPACE_DEFANSE_PROGRESS_V2\n";
    file << m_lumen << L"\n";
...
void SpaceDefanseGameImpl::SelectSaveSlot(int slot)
{
    const int safeSlot = std::clamp(slot, 0, kSaveSlotCount - 1);
...
void SpaceDefanseGameImpl::ResetProgressMemory()
{
...
void SpaceDefanseGameImpl::DeleteSelectedSaveSlot()
{
...
void SpaceDefanseGameImpl::ResetProgressData()
{
...
void SpaceDefanseGameImpl::ResetToTitle()
{
...
void SpaceDefanseGameImpl::ResetToMenu()
{
...
void SpaceDefanseGameImpl::ResetGame()
{
...
void SpaceDefanseGameImpl::StartDemoRun()
{
...
void SpaceDefanseGameImpl::Update(float dt)
{
...
void SpaceDefanseGameImpl::UpdateDemoMode(float dt)
{
...
void SpaceDefanseGameImpl::TriggerDebugClear()
{
    if (m_screen != GameScreen::Playing)
    {
        return;
...
void SpaceDefanseGameImpl::TriggerDebugUnlockAll()
{
...
void SpaceDefanseGameImpl::UpdateViewMetrics()
{
    if (!m_renderTarget)
    {
        m_viewScale = 1.0f;
...
void SpaceDefanseGameImpl::SetViewTransform(float worldCameraX, bool includeCameraShake)
{
    Vec2 shake = {};
...
Vec2 SpaceDefanseGameImpl::ClientToVirtual(Vec2 pos) const
{
...
void SpaceDefanseGameImpl::UpdateCamera(float dt)
{
    if (m_screen != GameScreen::Playing && m_screen != GameScreen::Result)
    {
        m_cameraTargetX = 0.0f;
