...
void SpaceDefenceGameImpl::UpdateEnemyDirector(float dt)
{
...
void SpaceDefenceGameImpl::UpdateDirectorPressure(float dt)
{
...
float SpaceDefenceGameImpl::DirectorSpawnMultiplier() const
{
    return std::max(0.84f, std::min(1.16f, 1.0f - m_directorPressure * 0.12f));
}

float SpaceDefenceGameImpl::BossTriggerHpRatio() const
{
...
void SpaceDefenceGameImpl::PromoteBossUnit(Unit& boss)
{
...
float SpaceDefenceGameImpl::BossDamageTakenScale(const Unit& boss) const
{
...
EnemyUnit SpaceDefenceGameImpl::PickStageEnemy(int value, int phase) const
{
...
EnemyUnit SpaceDefenceGameImpl::StageBossType() const
{
    switch (m_selectedStage)
    {
    case 0:
...
std::wstring SpaceDefenceGameImpl::StageEnemySummary() const
{
    switch (m_selectedStage)
    {
    case 0:
...
std::wstring SpaceDefenceGameImpl::CounterPlanSummary() const
{
...
float SpaceDefenceGameImpl::StageThreatRating() const
{
    const StageDefinition stage = CurrentStage();
...
float SpaceDefenceGameImpl::LoadoutPowerRating() const
{
    float total = 0.0f;
...
std::wstring SpaceDefenceGameImpl::BalanceAdvice() const
{
...
float SpaceDefenceGameImpl::GimmickInterval() const
{
    switch (m_selectedStage)
    {
    case 0: return 12.0f;
...
std::optional<std::reference_wrapper<Unit>> SpaceDefenceGameImpl::FindBossUnit()
{
...
std::optional<std::reference_wrapper<const Unit>> SpaceDefenceGameImpl::FindBossUnit() const
{
...
void SpaceDefenceGameImpl::UpdateBossPatterns(float dt)
{
...
void SpaceDefenceGameImpl::TriggerBossPattern(Unit& boss)
{
...
float SpaceDefenceGameImpl::EffectiveUnitRange(const Unit& unit) const
{
...
float SpaceDefenceGameImpl::StageMoveSpeedModifier(const Unit& unit) const
{
    float modifier = 1.0f;
...
void SpaceDefenceGameImpl::UpdateStageGimmicks(float dt)
{
...
void SpaceDefenceGameImpl::TriggerStageGimmick()
{
    const StageDefinition stage = CurrentStage();
...
void SpaceDefenceGameImpl::AddTelegraph(TelegraphKind kind, TelegraphShape shape, Vec2 start, Vec2 end, float radius, float width, float windup, float damage, D2D1_COLOR_F color)
{
    Telegraph telegraph;
    telegraph.kind = kind;
...
void SpaceDefenceGameImpl::UpdateTelegraphs(float dt)
{
...
void SpaceDefenceGameImpl::ExecuteTelegraph(const Telegraph& telegraph)
{
...
void SpaceDefenceGameImpl::ApplyAreaDamage(Vec2 center, float radius, float damage, D2D1_COLOR_F color)
{
    for (Unit& unit : m_units)
    {
...
void SpaceDefenceGameImpl::ApplyLineDamage(Vec2 start, Vec2 end, float width, float damage, D2D1_COLOR_F color)
{
...
void SpaceDefenceGameImpl::SpawnStageReinforcement(EnemyUnit type, float forwardOffset, bool elite)
{
...
int SpaceDefenceGameImpl::WalletUpgradeCost() const
{
    if (m_walletLevel >= 5)
    {
        return 0;
...
float SpaceDefenceGameImpl::WalletUnitBoost() const
{
    return 1.0f + static_cast<float>(m_walletLevel - 1) * 0.06f;
}

int SpaceDefenceGameImpl::UnitEnergyCost(PlayerUnit unit) const
{
    const UnitStats stats = PlayerStats(unit);
...
float SpaceDefenceGameImpl::UnitCooldown(PlayerUnit unit) const
{
    const UnitStats stats = PlayerStats(unit);
...
float SpaceDefenceGameImpl::WalletPulseInterval() const
{
    return std::max(7.2f, 18.0f - static_cast<float>(m_walletLevel - 1) * 2.5f);
}

bool SpaceDefenceGameImpl::HasLoadoutUnit(PlayerUnit unit) const
{
...
float SpaceDefenceGameImpl::SynergyHpMultiplier(PlayerUnit unit) const
{
...
float SpaceDefenceGameImpl::SynergyDamageMultiplier(PlayerUnit unit) const
{
...
float SpaceDefenceGameImpl::SynergyRangeMultiplier(PlayerUnit unit) const
{
...
float SpaceDefenceGameImpl::SynergySpeedMultiplier(PlayerUnit unit) const
{
    const bool rushPack = HasLoadoutUnit(PlayerUnit::Dash) && HasLoadoutUnit(PlayerUnit::Comet);
...
std::wstring SpaceDefenceGameImpl::SynergySummary() const
{
...
std::wstring SpaceDefenceGameImpl::GrowthRecommendation() const
{
...
void SpaceDefenceGameImpl::UpdateWalletPulse(float dt)
{
...
void SpaceDefenceGameImpl::TriggerWalletPulse(bool upgradeBurst)
{
...
void SpaceDefenceGameImpl::SpawnPlayer(PlayerUnit type)
{
...
void SpaceDefenceGameImpl::SpawnEnemy(EnemyUnit type, bool elite)
{
...
float SpaceDefenceGameImpl::RandomLaneY()
{
    std::uniform_real_distribution<float> dist(-58.0f, 58.0f);
    return kLaneY + dist(m_rng);
}

void SpaceDefenceGameImpl::UpdateUnits(float dt)
{
...
void SpaceDefenceGameImpl::SetUnitAnimState(Unit& unit, UnitAnimState state)
{
...
UnitAnimState SpaceDefenceGameImpl::ResolveAttackAnimState(const Unit& unit) const
{
...
int SpaceDefenceGameImpl::FindTargetIndex(const Unit& unit) const
{
    int bestIndex = -1;
...
bool SpaceDefenceGameImpl::IsBlocked(const Unit& unit) const
{
    const float dir = unit.team == Team::Player ? 1.0f : -1.0f;
...
bool SpaceDefenceGameImpl::IsEnemyBaseInRange(const Unit& unit) const
{
    if (unit.team == Team::Player)
    {
...
void SpaceDefenceGameImpl::BeginAttack(Unit& attacker, Vec2 targetPos)
{
...
void SpaceDefenceGameImpl::AddAttackVfx(const Unit& attacker, Vec2 targetPos, D2D1_COLOR_F color)
{
...
float SpaceDefenceGameImpl::AttackMatchupMultiplier(Team attackerTeam, int attackerKind, const Unit& target) const
{
...
void SpaceDefenceGameImpl::AddCounterFloatText(const Unit& target, float multiplier)
{
    if (multiplier > 1.08f)
    {
...
void SpaceDefenceGameImpl::AttackUnit(Unit& attacker, Unit& target)
{
...
void SpaceDefenceGameImpl::AttackBase(Unit& attacker)
{
...
void SpaceDefenceGameImpl::FireProjectile(Unit& attacker, const Unit& target)
{
    BeginAttack(attacker, target.pos);
...
void SpaceDefenceGameImpl::FireProjectileAtBase(Unit& attacker)
{
...
Vec2 SpaceDefenceGameImpl::ProjectileMuzzle(const Unit& attacker, Vec2 targetPos) const
{
    const Vec2 dir = Normalize(targetPos - attacker.pos);
...
void SpaceDefenceGameImpl::ConfigureProjectileVisual(Projectile& projectile, const Unit& attacker)
{
...
void SpaceDefenceGameImpl::AddProjectileImpact(const Projectile& projectile)
{
...
void SpaceDefenceGameImpl::AddMeleeClashVfx(const Unit& attacker, Vec2 targetPos, D2D1_COLOR_F color)
{
    const Vec2 dir = Normalize(targetPos - attacker.pos);
    const Vec2 normal = {-dir.y, dir.x};
...
void SpaceDefenceGameImpl::UpdateProjectiles(float dt)
{
...
std::optional<std::reference_wrapper<Unit>> SpaceDefenceGameImpl::FindUnitById(int id)
{
    for (Unit& unit : m_units)
    {
        if (unit.id == id)
...
void SpaceDefenceGameImpl::ApplyImpactReaction(Unit& target, Team sourceTeam, float damage, D2D1_COLOR_F color)
{
...
void SpaceDefenceGameImpl::DamageUnit(Unit& target, float damage, Team sourceTeam)
{
    if (!target.alive)
    {
        return;
...
void SpaceDefenceGameImpl::DamageBase(Team baseTeam, float damage, Vec2 source)
{
...
void SpaceDefenceGameImpl::ShakeUnit(Unit& unit, float duration)
{
    if (!m_hitShakeEnabled)
    {
        return;
...
void SpaceDefenceGameImpl::ShakeUnitById(int id, float duration)
{
...
void SpaceDefenceGameImpl::AddCameraTrauma(float amount)
{
    if (!m_hitShakeEnabled)
    {
        return;
...
void SpaceDefenceGameImpl::UpdateParticles(float dt)
{
...
void SpaceDefenceGameImpl::CleanupEntities()
{
    m_units.erase(
...
void SpaceDefenceGameImpl::TrySpawnPlayer(int index)
{
...
void SpaceDefenceGameImpl::TryUpgradeWallet()
{
    if (m_walletLevel >= 5)
    {
...
void SpaceDefenceGameImpl::TryFireCannon()
{
...
void SpaceDefenceGameImpl::DecreaseGameSpeed()
{
    m_gameSpeed = std::max(0.5f, m_gameSpeed - 0.5f);
    SetMessage(L"게임 속도 x" + ToWideFloat(m_gameSpeed));
}

void SpaceDefenceGameImpl::IncreaseGameSpeed()
{
    m_gameSpeed = std::min(3.0f, m_gameSpeed + 0.5f);
    SetMessage(L"게임 속도 x" + ToWideFloat(m_gameSpeed));
}

void SpaceDefenceGameImpl::AddParticle(Vec2 pos, Vec2 vel, float radius, float life, D2D1_COLOR_F color)
{
    AddParticleEx(pos, vel, radius, life, color, ParticleKind::Dot, 18.0f, 0.82f, 0.0f);
}

void SpaceDefenceGameImpl::AddParticleEx(Vec2 pos, Vec2 vel, float radius, float life, D2D1_COLOR_F color, ParticleKind kind, float gravity, float drag, float growth)
{
    Particle particle;
    particle.pos = pos;
    particle.vel = vel;
...
void SpaceDefenceGameImpl::AddRing(Vec2 pos, float maxRadius, float life, D2D1_COLOR_F color, float width)
{
    RingEffect ring;
    ring.pos = pos;
    ring.radius = 8.0f;
...
void SpaceDefenceGameImpl::AddBeam(Vec2 start, Vec2 end, float width, float life, D2D1_COLOR_F color)
{
    BeamEffect beam;
    beam.start = start;
    beam.end = end;
...
void SpaceDefenceGameImpl::AddSparkLines(Vec2 pos, D2D1_COLOR_F color, int count)
{
...
void SpaceDefenceGameImpl::AddImageVfx(ImageVfxKind kind, Vec2 pos, float size, float life, D2D1_COLOR_F color, float dir)
{
...
void SpaceDefenceGameImpl::AddBurst(Vec2 pos, D2D1_COLOR_F color, int count)
{
    std::uniform_real_distribution<float> angleDist(0.0f, kPi * 2.0f);
...
void SpaceDefenceGameImpl::AddDustPuff(Vec2 pos, D2D1_COLOR_F color, int count)
{
...
void SpaceDefenceGameImpl::AddDeathBurst(const Unit& unit)
{
...
void SpaceDefenceGameImpl::AddHitEffects(Vec2 pos, D2D1_COLOR_F color)
{
...
void SpaceDefenceGameImpl::AddFloatText(Vec2 pos, const std::wstring& text, D2D1_COLOR_F color, float life)
{
    FloatText item;
    item.pos = pos;
    item.text = text;
...
void SpaceDefenceGameImpl::AddUiPulse(Vec2 pos, D2D1_COLOR_F color, float maxRadius, float life)
{
    UiPulse pulse;
    pulse.pos = pos;
    pulse.radius = 8.0f;
