...
void SpaceDefenceGameImpl::ResetCombatFeedbackState()
{
    m_hitStopTimer = 0.0f;
    m_hitStopMax = 0.0f;
    m_slowMoTimer = 0.0f;
...
void SpaceDefenceGameImpl::UpdateCombatFeedbackTimers(float dt)
{
...
float SpaceDefenceGameImpl::CombatTimeScale() const
{
    if (m_hitStopTimer > 0.0f)
    {
...
float SpaceDefenceGameImpl::PostFxFeedbackIntensity() const
{
...
void SpaceDefenceGameImpl::TriggerHitStop(float holdDuration, float slowScale, float slowDuration)
{
...
void SpaceDefenceGameImpl::TriggerBossEntrance(Unit& boss, D2D1_COLOR_F color)
{
    boss.boss = true;
    boss.stunTimer = 0.0f;
...
void SpaceDefenceGameImpl::TriggerBossPhaseChange(Unit& boss, int phase)
{
...
float SpaceDefenceGameImpl::UnitKnockbackStep(const Unit& unit) const
{
    if (unit.boss)
    {
        return 0.22f;
...
void SpaceDefenceGameImpl::CheckUnitKnockback(Unit& target, Team sourceTeam)
{
...
void SpaceDefenceGameImpl::TriggerUnitKnockback(Unit& unit, Team sourceTeam, float strength, float stunDuration, bool force)
{
    if (!unit.alive)
    {
        return;
