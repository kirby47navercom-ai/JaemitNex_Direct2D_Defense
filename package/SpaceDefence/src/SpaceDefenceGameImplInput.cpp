...
void SpaceDefenceGameImpl::SetMessage(const std::wstring& message)
{
    m_message = message;
...
void SpaceDefenceGameImpl::OpenEscapeMenu()
{
    if (m_escapeMenuOpen)
    {
        return;
...
void SpaceDefenceGameImpl::CloseEscapeMenu()
{
    if (!m_escapeMenuOpen)
    {
        return;
...
void SpaceDefenceGameImpl::AdjustEscapeMenuSpeed(float delta)
{
...
UiSliderDragTarget SpaceDefenceGameImpl::SliderDragTargetAt(Vec2 pos) const
{
...
void SpaceDefenceGameImpl::BeginSliderDrag(Vec2 pos)
{
...
void SpaceDefenceGameImpl::UpdateSliderDrag(Vec2 pos, bool playFeedback)
{
    if (m_activeSliderDrag == UiSliderDragTarget::None)
    {
        return;
...
void SpaceDefenceGameImpl::EndSliderDrag()
{
    if (m_activeSliderDrag != UiSliderDragTarget::None)
    {
        SaveProgress();
...
void SpaceDefenceGameImpl::ApplySliderDragValue(UiSliderDragTarget target, Vec2 pos, bool playFeedback)
{
    switch (target)
    {
...
bool SpaceDefenceGameImpl::IsInteractivePoint(Vec2 pos) const
{
    if (m_escapeMenuOpen)
    {
...
void SpaceDefenceGameImpl::OnLeftClick(Vec2 pos)
{
    // Every screen owns its click handling. The shared rectangle helpers live in
    // SpaceDefenceGameImplLayout.cpp, so render and input stay pixel-aligned.
...
void SpaceDefenceGameImpl::OnMenuClick(Vec2 pos)
{
    for (int i = 0; i < kStageCount; ++i)
    {
        if (Contains(MenuStageRect(i), pos))
...
void SpaceDefenceGameImpl::OnBriefingClick(Vec2 pos)
{
...
void SpaceDefenceGameImpl::OnShopClick(Vec2 pos)
{
    if (Contains(ShopBackButtonRect(), pos))
    {
        ResetToMenu();
...
void SpaceDefenceGameImpl::OnOptionsClick(Vec2 pos)
{
...
void SpaceDefenceGameImpl::OnEscapeMenuClick(Vec2 pos)
{
    const auto sliderValue = [](D2D1_RECT_F rect, Vec2 point) {
        const float width = std::max(1.0f, rect.right - rect.left);
        return Clamp01((point.x - rect.left) / width);
...
void SpaceDefenceGameImpl::OnResultClick(Vec2 pos)
{
...
void SpaceDefenceGameImpl::TryBuyOrUpgradeUnit(PlayerUnit unit)
{
...
void SpaceDefenceGameImpl::SetLoadoutUnit(PlayerUnit unit)
{
...
void SpaceDefenceGameImpl::OnKeyDown(WPARAM key)
{
    if (m_escapeMenuOpen)
    {
