...
이 폴더의 에셋은 과제 기간 중 프로젝트에 맞춰 직접 생성하거나, 무료/CC0 범위의 리소스를 확인해 사용한 것이다.

## 배경과 컷신

- `backgrounds/stage_00_space_hd.jpg` ~ `stage_09_space_hd.jpg`
  - 스테이지별 고화질 우주 배경 레이어.
  - `tools/generate_quality_assets.py`로 직접 생성했다.
  - 배경은 실제 우주 공간처럼 보이게 만들고, 행성별 개성은 전투 무대 바닥 패턴과 색감으로 표현한다.
- `backgrounds/stage_00_space.png` ~ `stage_09_space.png`
  - 이전 버전의 스테이지 배경 레이어.
  - 현재 게임에서는 HD JPG 배경을 우선 사용한다.
- `cutins/prologue_story_art.jpg`
  - 프롤로그 스타워즈풍 스토리 장면에 섞어 쓰는 배경 아트.
- `cutins/ending_solar_route.jpg`
  - 태양 클리어 후 엔딩 장면에 쓰는 배경 아트.
...
- 적용 방식: 원본 PNG를 128x128 셀 기반 아틀라스로 재구성하고, 유닛 종류별 색/역할 차이를 보이게 보정했다.
...
- 현재 메인 렌더링은 Kenney 통합 아틀라스를 사용하며, 이 폴더는 백업/비교용으로 남겨두었다.

## 전투 VFX

- `vfx/*.png`
  - 게임에서 실제로 로드하는 전투 이펙트 시트.
  - `ImageVfxKind`에 따라 베기, 회복, 화염, 얼음, 전기, 물, 암흑, 산성, 땅, 연기, 성스러운 빛, 바람, 돌진, 폭발 계열을 다르게 재생한다.
  - 프레임 크기와 행/열은 코드의 `ImageVfxSpec`과 맞춰야 한다.
- `vfx/source_effects/`
  - 사용자가 제공한 이펙트 원본과 참고용 시트 모음.

## 무기

- `weapons/external_roles/*.png`
  - 플레이어/적 유닛 역할별 무기 이미지.
  - 유닛의 손 위치 기준으로 회전/이동하면서 수동 공격 애니메이션에 쓰인다.
