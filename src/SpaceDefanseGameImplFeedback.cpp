...
void SpaceDefanseGameImpl::ResetCombatFeedbackState()
{
    m_hitStopTimer = 0.0f;
    m_hitStopMax = 0.0f;
    m_slowMoTimer = 0.0f;
...
void SpaceDefanseGameImpl::UpdateCombatFeedbackTimers(float dt)
{
...
float SpaceDefanseGameImpl::CombatTimeScale() const
{
    if (m_hitStopTimer > 0.0f)
    {
...
float SpaceDefanseGameImpl::PostFxFeedbackIntensity() const
{
...
void SpaceDefanseGameImpl::TriggerHitStop(float holdDuration, float slowScale, float slowDuration)
{
...
void SpaceDefanseGameImpl::TriggerBossEntrance(Unit& boss, D2D1_COLOR_F color)
{
    boss.boss = true;
    boss.stunTimer = 0.0f;
...
void SpaceDefanseGameImpl::TriggerBossPhaseChange(Unit& boss, int phase)
{
...
float SpaceDefanseGameImpl::UnitKnockbackStep(const Unit& unit) const
{
    if (unit.boss)
    {
        return 0.22f;
...
void SpaceDefanseGameImpl::CheckUnitKnockback(Unit& target, Team sourceTeam)
{
...
void SpaceDefanseGameImpl::TriggerUnitKnockback(Unit& unit, Team sourceTeam, float strength, float stunDuration, bool force)
{
    if (!unit.alive)
    {
        return;
