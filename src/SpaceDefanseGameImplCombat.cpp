...
void SpaceDefanseGameImpl::UpdateEnemyDirector(float dt)
{
...
void SpaceDefanseGameImpl::UpdateDirectorPressure(float dt)
{
...
float SpaceDefanseGameImpl::DirectorSpawnMultiplier() const
{
    return std::max(0.84f, std::min(1.16f, 1.0f - m_directorPressure * 0.12f));
}

float SpaceDefanseGameImpl::BossTriggerHpRatio() const
{
...
void SpaceDefanseGameImpl::PromoteBossUnit(Unit& boss)
{
...
float SpaceDefanseGameImpl::BossDamageTakenScale(const Unit& boss) const
{
...
EnemyUnit SpaceDefanseGameImpl::PickStageEnemy(int value, int phase) const
{
...
EnemyUnit SpaceDefanseGameImpl::StageBossType() const
{
    switch (m_selectedStage)
    {
    case 0:
...
std::wstring SpaceDefanseGameImpl::StageEnemySummary() const
{
    switch (m_selectedStage)
    {
    case 0:
...
std::wstring SpaceDefanseGameImpl::CounterPlanSummary() const
{
...
float SpaceDefanseGameImpl::StageThreatRating() const
{
    const StageDefinition stage = CurrentStage();
...
float SpaceDefanseGameImpl::LoadoutPowerRating() const
{
    float total = 0.0f;
...
std::wstring SpaceDefanseGameImpl::BalanceAdvice() const
{
...
float SpaceDefanseGameImpl::GimmickInterval() const
{
    switch (m_selectedStage)
    {
    case 0: return 12.0f;
...
std::optional<std::reference_wrapper<Unit>> SpaceDefanseGameImpl::FindBossUnit()
{
...
std::optional<std::reference_wrapper<const Unit>> SpaceDefanseGameImpl::FindBossUnit() const
{
...
void SpaceDefanseGameImpl::UpdateBossPatterns(float dt)
{
...
void SpaceDefanseGameImpl::TriggerBossPattern(Unit& boss)
{
...
float SpaceDefanseGameImpl::EffectiveUnitRange(const Unit& unit) const
{
...
float SpaceDefanseGameImpl::StageMoveSpeedModifier(const Unit& unit) const
{
    float modifier = 1.0f;
...
void SpaceDefanseGameImpl::UpdateStageGimmicks(float dt)
{
...
void SpaceDefanseGameImpl::TriggerStageGimmick()
{
    const StageDefinition stage = CurrentStage();
...
void SpaceDefanseGameImpl::AddTelegraph(TelegraphKind kind, TelegraphShape shape, Vec2 start, Vec2 end, float radius, float width, float windup, float damage, D2D1_COLOR_F color)
{
    Telegraph telegraph;
    telegraph.kind = kind;
...
void SpaceDefanseGameImpl::UpdateTelegraphs(float dt)
{
...
void SpaceDefanseGameImpl::ExecuteTelegraph(const Telegraph& telegraph)
{
...
void SpaceDefanseGameImpl::ApplyAreaDamage(Vec2 center, float radius, float damage, D2D1_COLOR_F color)
{
    for (Unit& unit : m_units)
    {
...
void SpaceDefanseGameImpl::ApplyLineDamage(Vec2 start, Vec2 end, float width, float damage, D2D1_COLOR_F color)
{
...
void SpaceDefanseGameImpl::SpawnStageReinforcement(EnemyUnit type, float forwardOffset, bool elite)
{
...
int SpaceDefanseGameImpl::WalletUpgradeCost() const
{
    if (m_walletLevel >= 5)
    {
        return 0;
...
float SpaceDefanseGameImpl::WalletUnitBoost() const
{
    return 1.0f + static_cast<float>(m_walletLevel - 1) * 0.06f;
}

int SpaceDefanseGameImpl::UnitEnergyCost(PlayerUnit unit) const
{
    const UnitStats stats = PlayerStats(unit);
...
float SpaceDefanseGameImpl::UnitCooldown(PlayerUnit unit) const
{
    const UnitStats stats = PlayerStats(unit);
...
float SpaceDefanseGameImpl::WalletPulseInterval() const
{
    return std::max(7.2f, 18.0f - static_cast<float>(m_walletLevel - 1) * 2.5f);
}

bool SpaceDefanseGameImpl::HasLoadoutUnit(PlayerUnit unit) const
{
...
float SpaceDefanseGameImpl::SynergyHpMultiplier(PlayerUnit unit) const
{
...
float SpaceDefanseGameImpl::SynergyDamageMultiplier(PlayerUnit unit) const
{
...
float SpaceDefanseGameImpl::SynergyRangeMultiplier(PlayerUnit unit) const
{
...
float SpaceDefanseGameImpl::SynergySpeedMultiplier(PlayerUnit unit) const
{
    const bool rushPack = HasLoadoutUnit(PlayerUnit::Dash) && HasLoadoutUnit(PlayerUnit::Comet);
...
std::wstring SpaceDefanseGameImpl::SynergySummary() const
{
...
std::wstring SpaceDefanseGameImpl::GrowthRecommendation() const
{
...
void SpaceDefanseGameImpl::UpdateWalletPulse(float dt)
{
...
void SpaceDefanseGameImpl::TriggerWalletPulse(bool upgradeBurst)
{
...
void SpaceDefanseGameImpl::SpawnPlayer(PlayerUnit type)
{
...
float SpaceDefanseGameImpl::RandomLaneY()
{
    std::uniform_real_distribution<float> dist(-58.0f, 58.0f);
    return kLaneY + dist(m_rng);
}

void SpaceDefanseGameImpl::UpdateUnits(float dt)
{
...
void SpaceDefanseGameImpl::SetUnitAnimState(Unit& unit, UnitAnimState state)
{
...
UnitAnimState SpaceDefanseGameImpl::ResolveAttackAnimState(const Unit& unit) const
{
...
int SpaceDefanseGameImpl::FindTargetIndex(const Unit& unit) const
{
    int bestIndex = -1;
...
bool SpaceDefanseGameImpl::IsBlocked(const Unit& unit) const
{
    const float dir = unit.team == Team::Player ? 1.0f : -1.0f;
...
bool SpaceDefanseGameImpl::IsEnemyBaseInRange(const Unit& unit) const
{
    if (unit.team == Team::Player)
    {
...
void SpaceDefanseGameImpl::BeginAttack(Unit& attacker, Vec2 targetPos)
{
...
void SpaceDefanseGameImpl::AddAttackVfx(const Unit& attacker, Vec2 targetPos, D2D1_COLOR_F color)
{
...
float SpaceDefanseGameImpl::AttackMatchupMultiplier(Team attackerTeam, int attackerKind, const Unit& target) const
{
...
void SpaceDefanseGameImpl::AddCounterFloatText(const Unit& target, float multiplier)
{
    if (multiplier > 1.08f)
    {
...
void SpaceDefanseGameImpl::AttackUnit(Unit& attacker, Unit& target)
{
...
void SpaceDefanseGameImpl::AttackBase(Unit& attacker)
{
...
void SpaceDefanseGameImpl::FireProjectile(Unit& attacker, const Unit& target)
{
    BeginAttack(attacker, target.pos);
...
void SpaceDefanseGameImpl::FireProjectileAtBase(Unit& attacker)
{
...
Vec2 SpaceDefanseGameImpl::ProjectileMuzzle(const Unit& attacker, Vec2 targetPos) const
{
    const Vec2 dir = Normalize(targetPos - attacker.pos);
...
void SpaceDefanseGameImpl::ConfigureProjectileVisual(Projectile& projectile, const Unit& attacker)
{
...
void SpaceDefanseGameImpl::AddProjectileImpact(const Projectile& projectile)
{
...
void SpaceDefanseGameImpl::AddMeleeClashVfx(const Unit& attacker, Vec2 targetPos, D2D1_COLOR_F color)
{
    const Vec2 dir = Normalize(targetPos - attacker.pos);
    const Vec2 normal = {-dir.y, dir.x};
...
void SpaceDefanseGameImpl::UpdateProjectiles(float dt)
{
...
std::optional<std::reference_wrapper<Unit>> SpaceDefanseGameImpl::FindUnitById(int id)
{
    for (Unit& unit : m_units)
    {
        if (unit.id == id)
...
void SpaceDefanseGameImpl::ApplyImpactReaction(Unit& target, Team sourceTeam, float damage, D2D1_COLOR_F color)
{
...
void SpaceDefanseGameImpl::DamageUnit(Unit& target, float damage, Team sourceTeam)
{
    if (!target.alive)
    {
        return;
...
void SpaceDefanseGameImpl::DamageBase(Team baseTeam, float damage, Vec2 source)
{
...
void SpaceDefanseGameImpl::ShakeUnit(Unit& unit, float duration)
{
    if (!m_hitShakeEnabled)
    {
        return;
...
void SpaceDefanseGameImpl::ShakeUnitById(int id, float duration)
{
...
void SpaceDefanseGameImpl::AddCameraTrauma(float amount)
{
    if (!m_hitShakeEnabled)
    {
        return;
...
void SpaceDefanseGameImpl::UpdateParticles(float dt)
{
...
void SpaceDefanseGameImpl::CleanupEntities()
{
    m_units.erase(
...
void SpaceDefanseGameImpl::TrySpawnPlayer(int index)
{
...
void SpaceDefanseGameImpl::TryUpgradeWallet()
{
    if (m_walletLevel >= 5)
    {
...
void SpaceDefanseGameImpl::TryFireCannon()
{
...
void SpaceDefanseGameImpl::DecreaseGameSpeed()
{
    m_gameSpeed = std::max(0.5f, m_gameSpeed - 0.5f);
    SetMessage(L"게임 속도 x" + ToWideFloat(m_gameSpeed));
}

void SpaceDefanseGameImpl::IncreaseGameSpeed()
{
    m_gameSpeed = std::min(3.0f, m_gameSpeed + 0.5f);
    SetMessage(L"게임 속도 x" + ToWideFloat(m_gameSpeed));
}

void SpaceDefanseGameImpl::AddParticle(Vec2 pos, Vec2 vel, float radius, float life, D2D1_COLOR_F color)
{
    AddParticleEx(pos, vel, radius, life, color, ParticleKind::Dot, 18.0f, 0.82f, 0.0f);
}

void SpaceDefanseGameImpl::AddParticleEx(Vec2 pos, Vec2 vel, float radius, float life, D2D1_COLOR_F color, ParticleKind kind, float gravity, float drag, float growth)
{
    Particle particle;
    particle.pos = pos;
    particle.vel = vel;
...
void SpaceDefanseGameImpl::AddRing(Vec2 pos, float maxRadius, float life, D2D1_COLOR_F color, float width)
{
    RingEffect ring;
    ring.pos = pos;
    ring.radius = 8.0f;
...
void SpaceDefanseGameImpl::AddBeam(Vec2 start, Vec2 end, float width, float life, D2D1_COLOR_F color)
{
    BeamEffect beam;
    beam.start = start;
    beam.end = end;
...
void SpaceDefanseGameImpl::AddSparkLines(Vec2 pos, D2D1_COLOR_F color, int count)
{
...
void SpaceDefanseGameImpl::AddImageVfx(ImageVfxKind kind, Vec2 pos, float size, float life, D2D1_COLOR_F color, float dir)
{
...
void SpaceDefanseGameImpl::AddBurst(Vec2 pos, D2D1_COLOR_F color, int count)
{
    std::uniform_real_distribution<float> angleDist(0.0f, kPi * 2.0f);
...
void SpaceDefanseGameImpl::AddDustPuff(Vec2 pos, D2D1_COLOR_F color, int count)
{
...
void SpaceDefanseGameImpl::AddDeathBurst(const Unit& unit)
{
...
void SpaceDefanseGameImpl::AddHitEffects(Vec2 pos, D2D1_COLOR_F color)
{
...
void SpaceDefanseGameImpl::AddFloatText(Vec2 pos, const std::wstring& text, D2D1_COLOR_F color, float life)
{
    FloatText item;
    item.pos = pos;
    item.text = text;
...
void SpaceDefanseGameImpl::AddUiPulse(Vec2 pos, D2D1_COLOR_F color, float maxRadius, float life)
{
    UiPulse pulse;
    pulse.pos = pos;
    pulse.radius = 8.0f;
