...
SpaceDefenceGameImpl::~SpaceDefenceGameImpl()
{
...
HRESULT SpaceDefenceGameImpl::Initialize()
{
    ...
    const wchar_t className[] = L"SpaceDefenceWindow";
    WNDCLASSEXW wc = {};
    wc.cbSize = sizeof(wc);
    wc.lpfnWndProc = SpaceDefenceGameImpl::WndProc;
...
LRESULT CALLBACK SpaceDefenceGameImpl::WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    if (message == WM_NCCREATE)
    {
        auto* create = reinterpret_cast<CREATESTRUCTW*>(lParam);
        auto* app = reinterpret_cast<SpaceDefenceGameImpl*>(create->lpCreateParams);
        SetWindowLongPtrW(hwnd, GWLP_USERDATA, reinterpret_cast<LONG_PTR>(app));
        app->m_hwnd = hwnd;
    }

    auto* app = reinterpret_cast<SpaceDefenceGameImpl*>(GetWindowLongPtrW(hwnd, GWLP_USERDATA));
    if (app)
    {
...
LRESULT SpaceDefenceGameImpl::HandleMessage(UINT message, WPARAM wParam, LPARAM lParam)
{
    switch (message)
    {
    case WM_DESTROY:
...
HRESULT SpaceDefenceGameImpl::CreateTextFormats()
{
...
HRESULT SpaceDefenceGameImpl::CreateDeviceResources()
{
    if (m_renderTarget)
    {
        return S_OK;
...
void SpaceDefenceGameImpl::DiscardDeviceResources()
{
    DiscardBitmapAssets();
    m_brush.Reset();
...
void SpaceDefenceGameImpl::RegisterPrivateFonts()
{
...
void SpaceDefenceGameImpl::UnregisterPrivateFonts()
{
...
std::wstring SpaceDefenceGameImpl::ExecutableDir() const
{
...
std::wstring SpaceDefenceGameImpl::AssetPath(const std::wstring& relativePath) const
{
...
bool SpaceDefenceGameImpl::StartMusicNow(const std::wstring& absolutePath, bool loop, float fadeLevel)
{
...
void SpaceDefenceGameImpl::PlayMusicTrack(const std::wstring& relativePath, float fadeSeconds, bool loop)
{
...
void SpaceDefenceGameImpl::StartBackgroundMusic()
{
...
std::wstring SpaceDefenceGameImpl::StageMusicPath(int stageIndex) const
{
...
void SpaceDefenceGameImpl::StartStageMusic()
{
    PlayMusicTrack(StageMusicPath(m_selectedStage), 0.70f, true);
    StartDangerMusicLayer();
}

std::wstring SpaceDefenceGameImpl::ResultMusicPath(bool victory) const
{
    return victory ? L"assets\\music\\result_victory.wav" : L"assets\\music\\result_defeat.wav";
}

void SpaceDefenceGameImpl::StartResultMusic(bool victory)
{
    StopDangerMusicLayer();
    PlayMusicTrack(ResultMusicPath(victory), 0.62f, true);
}

void SpaceDefenceGameImpl::PlayMusicStinger(const std::wstring& relativePath, float volumeScale)
{
...
void SpaceDefenceGameImpl::StartDangerMusicLayer()
{
...
void SpaceDefenceGameImpl::StopDangerMusicLayer()
{
...
float SpaceDefenceGameImpl::MusicDangerTarget() const
{
...
void SpaceDefenceGameImpl::UpdateMusicSystem(float dt)
{
    if (m_musicFadingOut)
    {
        m_musicFadeTimer += dt;
...
void SpaceDefenceGameImpl::SyncMusicVolume()
{
...
float SpaceDefenceGameImpl::VolumeForSfxKind(SfxKind kind) const
{
    // 메뉴 클릭음은 UI 볼륨, 전투에서 나는 나머지 소리는 효과음 볼륨을 따른다.
    return kind == SfxKind::Ui ? m_uiVolume : m_sfxVolume;
}

void SpaceDefenceGameImpl::PlaySfx(SfxKind kind, float minGapSeconds)
{
...
void SpaceDefenceGameImpl::PlaySfxAt(SfxKind kind, float worldX, float minGapSeconds, float volumeScale)
{
    std::wstring fileName;
    switch (kind)
    {
...
void SpaceDefenceGameImpl::PlaySfxFile(const std::wstring& relativeFileName, SfxKind throttleKind, float minGapSeconds)
{
...
void SpaceDefenceGameImpl::PlaySfxFileAt(const std::wstring& relativeFileName, SfxKind throttleKind, float worldX, float minGapSeconds, float volumeScale)
{
    const float volume = VolumeForSfxKind(throttleKind);
    if (volume <= 0.001f)
    {
...
std::wstring SpaceDefenceGameImpl::AttackSfxPath(const Unit& attacker) const
{
...
float SpaceDefenceGameImpl::AttackSfxVolumeScale(const Unit& attacker) const
{
...
void SpaceDefenceGameImpl::PlayAttackSfxAt(const Unit& attacker, float minGapSeconds)
{
...
void SpaceDefenceGameImpl::AdjustSfxVolume(float delta)
{
...
void SpaceDefenceGameImpl::AdjustUiVolume(float delta)
{
...
void SpaceDefenceGameImpl::AdjustBgmVolume(float delta)
{
...
void SpaceDefenceGameImpl::ResetAudioVolumes()
{
...
HRESULT SpaceDefenceGameImpl::LoadBitmapFromFile(const std::wstring& path, ID2D1Bitmap** bitmap) const
{
...
void SpaceDefenceGameImpl::LoadBitmapAssets()
{
    ...
    LoadBitmapFromFile(AssetPath(L"assets\\ui\\space_defence_ui_atlas.png"), m_uiAtlas.ReleaseAndGetAddressOf());

    m_bitmapAssetsLoaded = true;
}

void SpaceDefenceGameImpl::DiscardBitmapAssets()
{
    m_vfxAtlas.Reset();
...
const StageDefinition SpaceDefenceGameImpl::CurrentStage() const
{
    return GetStageDefinition(m_selectedStage);
}

int SpaceDefenceGameImpl::UnitIndex(PlayerUnit unit) const
{
    return std::max(0, std::min(kRosterCount - 1, static_cast<int>(unit)));
}

bool SpaceDefenceGameImpl::IsUnitUnlocked(PlayerUnit unit) const
{
    return m_unitUnlocked[UnitIndex(unit)];
}

int SpaceDefenceGameImpl::UnitLevel(PlayerUnit unit) const
{
    return m_unitLevels[UnitIndex(unit)];
}

bool SpaceDefenceGameImpl::IsUnitEvolved(PlayerUnit unit) const
{
    return IsUnitUnlocked(unit) && UnitLevel(unit) >= kMaxUnitLevel;
}

std::wstring SpaceDefenceGameImpl::UnitDisplayName(PlayerUnit unit) const
{
    const UnitStats base = GetPlayerStats(unit);
    return IsUnitEvolved(unit) ? L"진화 " + base.name : base.name;
}

UnitStats SpaceDefenceGameImpl::PlayerStats(PlayerUnit unit) const
{
...
int SpaceDefenceGameImpl::UnitUnlockCost(PlayerUnit unit) const
{
...
int SpaceDefenceGameImpl::UnitUpgradeCost(PlayerUnit unit) const
{
    const int level = UnitLevel(unit);
...
int SpaceDefenceGameImpl::StageClearReward(bool firstClear) const
{
...
float SpaceDefenceGameImpl::DifficultyThreatMultiplier() const
{
    switch (m_difficulty)
    {
...
float SpaceDefenceGameImpl::DifficultyRewardMultiplier() const
{
    switch (m_difficulty)
    {
    case Difficulty::Easy:
...
std::wstring SpaceDefenceGameImpl::DifficultyLabel() const
{
    switch (m_difficulty)
    {
    case Difficulty::Easy:
...
bool SpaceDefenceGameImpl::IsStageUnlocked(int index) const
{
...
int SpaceDefenceGameImpl::HighestUnlockedStage() const
{
    int highest = 0;
...
void SpaceDefenceGameImpl::SelectStage(int index)
{
...
void SpaceDefenceGameImpl::GrantStageReward()
{
...
bool SpaceDefenceGameImpl::HasAnyProgressFile() const
{
    const auto hasGameplayProgress = [](const std::wstring& path) {
        ...
        file >> header;
        int lumen = 0;
        if (header == L"SPACE_DEFENCE_PROGRESS_V2")
        {
            file >> lumen;
        }
        else
...
void SpaceDefenceGameImpl::BeginStoryCrawl(bool autoContinueToMenu, GameScreen returnScreen, bool returnToEscapeMenu)
{
...
void SpaceDefenceGameImpl::FinishStoryCrawl()
{
    m_storyTimer = 0.0f;
    m_sceneTransitionTimer = m_sceneTransitionMax;
...
void SpaceDefenceGameImpl::BeginEndingScene()
{
...
void SpaceDefenceGameImpl::FinishEndingScene()
{
    m_endingTimer = 0.0f;
...
std::wstring SpaceDefenceGameImpl::ProgressPath() const
{
    return ProgressPath(m_saveSlot);
}

std::wstring SpaceDefenceGameImpl::ProgressPath(int slot) const
{
    ...
    const std::wstring fileName = L"space_defence_progress_slot" + ToWideInt(safeSlot + 1) + L".txt";
    if (slash == std::wstring::npos)
    {
        return fileName;
    }
...
std::wstring SpaceDefenceGameImpl::LegacyProgressPath() const
{
    wchar_t path[MAX_PATH] = {};
    GetModuleFileNameW(nullptr, path, MAX_PATH);
    std::wstring fullPath = path;
    ...
    if (slash == std::wstring::npos)
    {
        return L"space_defence_progress.txt";
    }
    return fullPath.substr(0, slash + 1) + L"space_defence_progress.txt";
}

std::wstring SpaceDefenceGameImpl::SaveSlotLabel() const
{
    return L"슬롯 " + ToWideInt(m_saveSlot + 1);
}

void SpaceDefenceGameImpl::LoadProgress()
{
    LoadProgress(m_saveSlot);
}

void SpaceDefenceGameImpl::LoadProgress(int slot)
{
    ...
    std::wstring header;
    file >> header;
    const bool version2 = header == L"SPACE_DEFENCE_PROGRESS_V2";
    if (version2)
    {
        file >> m_lumen;
    }
...
void SpaceDefenceGameImpl::SaveProgress()
{
...
void SpaceDefenceGameImpl::SaveProgressToSlot(int slot) const
{
    ...
    file << L"SPACE_DEFENCE_PROGRESS_V2\n";
    file << m_lumen << L"\n";
...
void SpaceDefenceGameImpl::SelectSaveSlot(int slot)
{
    const int safeSlot = std::clamp(slot, 0, kSaveSlotCount - 1);
...
void SpaceDefenceGameImpl::ResetProgressMemory()
{
...
void SpaceDefenceGameImpl::DeleteSelectedSaveSlot()
{
...
void SpaceDefenceGameImpl::ResetProgressData()
{
...
void SpaceDefenceGameImpl::ResetToTitle()
{
...
void SpaceDefenceGameImpl::ResetToMenu()
{
...
void SpaceDefenceGameImpl::ResetGame()
{
...
void SpaceDefenceGameImpl::StartDemoRun()
{
...
void SpaceDefenceGameImpl::Update(float dt)
{
...
void SpaceDefenceGameImpl::UpdateDemoMode(float dt)
{
...
void SpaceDefenceGameImpl::TriggerDebugClear()
{
    if (m_screen != GameScreen::Playing)
    {
        return;
...
void SpaceDefenceGameImpl::TriggerDebugUnlockAll()
{
...
void SpaceDefenceGameImpl::UpdateViewMetrics()
{
    if (!m_renderTarget)
    {
        m_viewScale = 1.0f;
...
void SpaceDefenceGameImpl::SetViewTransform(float worldCameraX, bool includeCameraShake)
{
    Vec2 shake = {};
