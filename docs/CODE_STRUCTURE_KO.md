...
이 문서는 `Pawline Defense`의 코드를 처음 읽는 사람이 어디서부터 보면 되는지 정리한 안내서다. 게임잼 리포트나 코드 설명을 쓸 때도 이 흐름을 따라가면 된다.

## 전체 흐름

1. `src/main.cpp`가 Win32 창을 만들고 `PawlineGame`을 실행한다.
2. `PawlineGame`은 외부에 보이는 얇은 껍데기이고, 실제 게임 상태는 `PawlineGameImpl`이 가진다.
3. 매 프레임 `DeltaTimer`가 델타타임을 계산한다.
4. `PawlineGameImplCore.cpp`의 업데이트 루프가 입력, 전투, VFX, 사운드, 저장 상태를 갱신한다.
5. `PawlineGameImplRender.cpp`가 현재 화면 상태에 맞춰 타이틀, 메뉴, 브리핑, 전투, 결과 화면을 그린다.

## 파일별 역할

- `PawlineGameImpl.h`: 전투 유닛, 투사체, 파티클, 저장 데이터, 화면 상태 같은 핵심 구조체와 멤버 변수를 모아둔 중심 헤더다.
- `PawlineGameImplCore.cpp`: 초기화, 리소스 로딩, 저장/불러오기, 전체 프레임 업데이트, 효과음 호출을 담당한다.
- `PawlineGameImplCombat.cpp`: 유닛 이동, 타겟 탐색, 공격, 피해 처리, 스폰, 스테이지 기믹, 월렛, 문빔 캐논을 담당한다.
- `PawlineGameImplInput.cpp`: 키보드와 마우스 입력을 화면별로 해석한다.
- `PawlineGameImplLayout.cpp`: 버튼과 카드의 좌표를 한곳에서 관리한다. 입력 판정과 렌더링이 같은 좌표를 쓰게 하기 위한 파일이다.
- `PawlineGameImplRender.cpp`: Direct2D 렌더링, UI, 캐릭터, 무기, 배경, 이미지 VFX, 후처리 느낌의 빛 효과를 담당한다.
...
화면을 바꾸는 코드는 대부분 `PawlineGameImplInput.cpp`와 `PawlineGameImplCore.cpp`에 있다. 화면 전환 연출은 커튼 타이머와 사운드 이벤트를 통해 처리한다.
...
UI 위치가 이상하면 먼저 `PawlineGameImplLayout.cpp`의 좌표 함수를 확인한다. 전투 중 월드 위치가 이상하면 `PawlineGameImplCombat.cpp`의 `pos` 계산과 `PawlineGameImplRender.cpp`의 카메라 변환을 같이 확인한다.
...
2. 전투 규칙이면 `PawlineGameImplCombat.cpp`에 구현한다.
3. 클릭 영역이 필요하면 `PawlineGameImplLayout.cpp`에 좌표 함수를 만든다.
4. 화면 표시가 필요하면 `PawlineGameImplRender.cpp`에 렌더 함수를 추가한다.
